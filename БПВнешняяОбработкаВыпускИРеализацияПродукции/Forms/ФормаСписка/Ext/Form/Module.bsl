...
Процедура ЗаполнитьНоменклатуруИКонтрагентовНаСервере()
	...
	ЗаполнитьСправочники(ЭтотОбъект, ЭлементыСправочников.Номенклатура);
...
Процедура ПоказатьДокументы(Команда)
	...
	Объект.ОтображаемаяПанель = ?(Объект.ОтображаемаяПанель = СтраницаНоменклатура, СтраницаДокументы, СтраницаНоменклатура);
	УстановитьПанель(ЭтотОбъект);
...
Функция ПриСозданииНаСервере_УстановитьИнтерфейс()
	...
	Элементы.НадписьПубликация.Заголовок = ОбщиеПараметры.НадписьПубликация;
...
Процедура УстановитьПанель(Форма)
	...
	СтраницаНоменклатура = Элементы.ОтображаемаяПанель.СписокВыбора[0].Значение;
	Пометка = ?(Форма.Объект.ОтображаемаяПанель = СтраницаНоменклатура, Ложь, Истина);
	Элементы.ПоказатьДокументы.Пометка = Пометка;
